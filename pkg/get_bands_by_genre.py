...
async def get_bands(message: Message, state: FSMContext):
    ...
    bands = response.json()
    if len(bands) == 0:
        await message.answer("Группы, исполняющие в данном жанре не найдены. Возможно, вы совершили опечатку, или жанр ещё не добавлен")
        await state.clear()
        return
    ...
    await message.answer("Это все существующие в нашей базе данных группы, исполняющие в данном жанре")
    await state.clear()
